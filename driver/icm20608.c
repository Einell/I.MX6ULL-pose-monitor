...
static s32 icm20608_write_regs(struct icm20608_dev *dev, u8 reg, u8 *buf, u8 len)
{
	...
	*txdata = reg & ~0x80;	/* 首寄存器地址bit8清零 */
    memcpy(txdata+1, buf, len);	/* 把寄存器拷贝到txdata里，等待发送 */
...
/*
 * @description	: 向icm20608指定寄存器写入指定的值
 * @param - dev:  icm20608设备
 * @param - reg:  要写的寄存器
 * @param - data: 要写入的值
 * @return   :    无
